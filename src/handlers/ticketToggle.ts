import {
  ...
  export async function handleTicketToggleCommand(interaction: ChatInputCommandInteraction) {
    ...
    if (mode === "channel") {
      ...
      const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId("ticket_toggle_ping_yes").setLabel("Yes").setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId("ticket_toggle_ping_no").setLabel("No").setStyle(ButtonStyle.Danger)
      );
  ...
  export async function handleTicketToggleButtonInteraction(interaction: ButtonInteraction) {
    ...
    if (customId === "ticket_toggle_ping_yes" || customId === "ticket_toggle_ping_no") {
      ...
      const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId(`ticket_toggle_confirm_${pingChoice}`).setLabel("Confirm").setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId("ticket_toggle_cancel").setLabel("Cancel").setStyle(ButtonStyle.Danger)
      );
